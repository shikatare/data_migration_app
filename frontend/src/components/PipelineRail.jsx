...
export default function PipelineRail({ statuses, sourceLabel, targetLabel }) {
  const STAGES = [
    { key: "extract", label: "Extract", sub: `${sourceLabel || "Source"} DDL pull`, tint: "oracle" },
    { key: "convert", label: "Convert", sub: "LLM conversion", tint: "amber" },
    { key: "validate", label: "Validate", sub: "Syntax + diff", tint: "amber" },
    { key: "deploy", label: "Deploy", sub: `${targetLabel || "Target"} apply`, tint: "snowflake" },
  ];
      ...
      {STAGES.map((stage) => {
        ...
        return (
          <div className="rail-stage" key={stage.key}>
            <div className={`rail-card rail-tint-${stage.tint} rail-status-${status}`}>
